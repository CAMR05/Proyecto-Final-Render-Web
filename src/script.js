...
updateCameraPosition() // Ejecutar al inicio
...
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)) // Importante para rendimiento en móvil
...
window.addEventListener('resize', () => {
    ...
    // Reajustar posición si rotan la pantalla y no ha empezado
    if(!isExperienceActive) updateCameraPosition()
})
...
gltfLoader.load('/models/mountain_and_river_scroll/scene.gltf', (gltf) => {
...
const loadCarousel = () => {
    ...
    modelsList.forEach((modelData, index) => {
        ...
        gltfLoader.load(modelData.path, (gltf) => {
            ...
            model.traverse((c) => { if(c.isMesh) { c.material.envMap = envMap; c.castShadow = true } })
            model.position.y = -1 
            itemPivot.add(model)
        ...
        if (modelData.textPath) {
            gltfLoader.load(modelData.textPath, (gltf) => {
                ...
                textModel.traverse((child) => { if(child.isMesh) child.material.side = THREE.DoubleSide })
...
if (startButton) {
    startButton.addEventListener('click', () => {
        ...
        const tl = gsap.timeline({ onComplete: () => { isExperienceActive = true } })
...
window.addEventListener('touchmove', (e) => {
    ...
    scrollY -= deltaX * 0.008 // Sensibilidad táctil
    gsap.to(carouselGroup.rotation, { duration: 1.0, ease: 'power2.out', y: scrollY })
    touchStartX = touchX
})
